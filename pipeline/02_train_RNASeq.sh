...
#SBATCH --nodes=1
#SBATCH --ntasks=24
#SBATCH --mem 128gb -p intel,batch
...
IFS=,
tail -n +2 $SAMPLEFILE | sed -n ${N}p | while read SPECIES STRAIN PHYLUM BIOPROJECT BIOSAMPLE LOCUSTAG
do
    ...
    funannotate train -i $MASKED -o $ODIR/$BASE \
  	...
  	--cpus $CPUS --memory $MEM  --pasa_db mysql  --trinity $RNAFOLDER/$SPECIESNOSPACE/assembled.fasta \
